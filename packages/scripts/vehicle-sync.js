...
async function updateVehiclePosition(vehicleId, latitude, longitude, bearing) {
  try {
    const result = await convex.mutation(
      api.gtfs.updateVehiclePosition({
        vehicleId,
        latitude,
        longitude,
        bearing,
    ...
    console.log(
      `Updated vehicle ${vehicleId} position: ${latitude}, ${longitude}, bearing: ${bearing}`,
    )
    return result
  } catch (error) {
  ...
  switch (command) {
    ...
    case 'list':
      getCurrentVehiclePositions()
        .then((vehicles) => {
          ...
          vehicles.forEach((v) => {
            console.log(
              `  Vehicle ${v.id}: ${v.latitude}, ${v.longitude}, ${v.bearing}°`,
            )
          })
        })
        .catch((error) => {
    ...
    case 'update': {
      if (process.argv.length < 4) {
        console.log(
          'Usage: node vehicle-sync.js update <vehicleId> <latitude> <longitude> [bearing]',
        )
        process.exit(1)
      }
